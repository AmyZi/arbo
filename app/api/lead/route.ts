...
export async function POST(request: Request) {
  try {
    ...
    const {
      ...
      plan,
      message,
      // Honeypot field — real visitors never see or fill this (see the
      // form component). Any value here means it's a bot.
      company_url,
    ...
    if (company_url) {
      console.warn('Blocked lead: honeypot filled', { ip });
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        from: 'Growth101 Leads <onboarding@resend.dev>',
        to,
        reply_to: email,
        subject: `New lead: ${name}${business ? ` (${business})` : ''}`,
        html: `
          <h2>New lead from growth101.com</h2>
